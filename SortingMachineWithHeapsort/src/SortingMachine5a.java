...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (last - top > 0) {
            ...
            if (leftExists && rightExists) {
                rightBigger = order.compare(array[left], array[right]) < 1;
            }

            if (rightExists && rightBigger
                    && order.compare(array[top], array[left]) > 0) {
                exchangeEntries(array, top, left);
                siftDown(array, left, last, order);

            } else if (rightExists && !rightBigger
                    && order.compare(array[top], array[right]) > 0) {
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        int left = 2 * top + 1;
        int right = left + 1;
        boolean leftExists = left < array.length;
        boolean rightExists = right < array.length;
