...
public final class StringReassembly {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        // Tests to see if any value in set has str as a substring
        for (String x : strSet) {
            if (x.contains(str)) {
                containsStr = true;
            }
        ...
        //if not add str to set
        if (!containsStr) {
            strSet.add(str);
        }

        //make temp set and transfer all values to temp
        Set<String> temp = strSet.newInstance();
        temp.transferFrom(strSet);

        //adds values of temp back to original set
        //(only ones that aren't substrings of str)
        while (!containsStr && temp.size() > 1) {
            String current = temp.removeAny();
            if (!str.contains(current)) {
                strSet.add(current);
            }
        }

        //transfer from temp to original set
        strSet.add(temp);
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //until end of file add lines without substrings in them
        while (!input.atEOS()) {
            String current = input.nextLine();
            addToSetAvoidingSubstrings(lines, current);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        //prints every character in string but replaces ~ with a new line
